...
// Música de fondo
final class MusicaGeo: ObservableObject {
    private var player: AVAudioPlayer?
    @Published var isPlaying: Bool = false
    @Published var volume: Float = 0.5
    
    func iniciarMusica() {
        guard let url = Bundle.main.url(forResource: "geo", withExtension: "mp3") else {
    ...
    func cargarPreguntasGeoExplora() {
            ...
            // rango ampliado de edades
                ...
                if filtradas.isEmpty {
                    self.errorMessage = "No hay preguntas de GeoExplora disponibles para idioma \(self.idiomaUsuario) y edad \(edadActual)"
                    self.isLoading = false
                    return
                }
...
struct GEOEXPLORAVIEW: View {
    ...
    @StateObject private var musica = MusicaGeo()
