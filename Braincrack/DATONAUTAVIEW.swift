...
// Música de fondo
final class MusicaDati: ObservableObject {
    private var player: AVAudioPlayer?
    @Published var isPlaying: Bool = false
    @Published var volume: Float = 0.5
    
    func iniciarMusica() {
        guard let url = Bundle.main.url(forResource: "dati", withExtension: "mp3") else {
...
final class DataNautaViewModel: ObservableObject {
    ...
    // preguntas cargarlas y condicional
    func cargarPreguntasDataNauta() {
            ...
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.3) { [weak self] in
                ...
                if filtradas.isEmpty {
                    self.errorMessage = "No hay preguntas de Dati Nauta disponibles para idioma \(self.idiomaUsuario) y edad \(edadActual)"
                    self.isLoading = false
                    return
                }
...
struct DATONAUTAVIEW: View {
    ...
    @StateObject private var musica = MusicaDati()
