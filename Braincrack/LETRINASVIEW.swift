...
final class LetrinasQuestionsService {
    func cargarPreguntasDesdeJSON(idioma: String) -> [PreguntaL] {
        ...
        do {
            ...
            let preguntas = try JSONDecoder().decode([PreguntaL].self, from: data)
...
enum FondoLetrinas {
    ...
    static func fondo(para tema: String, gameOver: Bool) -> FondoLetrinas {
        if gameOver {
            return .GAME_OVER
        }
        
        switch tema {
        case "Latrines", "Latrinen", "Letrinas":
            return .LETRINAS_NORMAL
        default:
            return .DEFAULT
        }
    }
...
final class LetrinasViewModel: ObservableObject {
    ...
    @Published var preguntaActual: PreguntaL?
    ...
    private let questionsService = LetrinasQuestionsService()
    private let soundManager = Sonidos()
    private var preguntasDisponibles: [PreguntaL] = []
    ...
    func cargarDatosUsuario() {
        ...
        docRef.getDocument { [weak self] (document, error) in
            ...
            // Cargar Score
            if let score = data["score_words"] as? Int {
                self.mejorScore = score
            } else if let scoreDouble = data["score_words"] as? Double {
                self.mejorScore = Int(scoreDouble)
            } else {
                self.mejorScore = 0
            }
            
            print("👤 Edad: \(self.edadUsuario ?? 9), Mejor Score Letrinas: \(self.mejorScore)")
...
//fondos
    ...
    private func fondoDinamico() -> some View {
        ...
        let fondoCase = FondoLetrinas.fondo(para: tema, gameOver: vm.gameOver)
