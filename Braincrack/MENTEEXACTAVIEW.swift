...
// Música de fondo
final class MusicaExacta: ObservableObject {
    private var player: AVAudioPlayer?
    @Published var isPlaying: Bool = false
    @Published var volume: Float = 0.5
    
    func iniciarMusica() {
        guard let url = Bundle.main.url(forResource: "mente", withExtension: "mp3") else {
...
struct PreguntaM: Codable, Identifiable {
    let id: Int
    let pregunta: String
    let resp: [String]
    let respc: String
...
final class MenteQuestionsService {
    func cargarPreguntasDesdeJSON(idioma: String) -> [PreguntaM] {
        var fileName: String
        ...
        do {
            ...
            let preguntas = try JSONDecoder().decode([PreguntaM].self, from: data)
...
enum FondoMente {
    ...
    static func fondo(para tema: String, gameOver: Bool) -> FondoMente {
        if gameOver {
            return .GAME_OVER
        }
        
        switch tema {
        case "Exact Mind", "Exakter Verstand", "Mente Exacta":
            return .MENTE_NORMAL
        default:
            return .DEFAULT
        }
...
// MARK: - 4. ViewModel
final class MenteViewModel: ObservableObject {
    
    @Published var preguntas: [PreguntaM] = []
    @Published var preguntaActual: PreguntaM?
    ...
    private let questionsService = MenteQuestionsService()
    private let soundManager = Sonidos()
    private var preguntasDisponibles: [PreguntaM] = []
    ...
    func cargarDatosUsuario() {
        ...
        docRef.getDocument { [weak self] (document, error) in
            ...
            // Cargar Score
            if let score = data["score_math"] as? Int {
                self.mejorScore = score
            } else if let scoreDouble = data["score_math"] as? Double {
                self.mejorScore = Int(scoreDouble)
            } else {
                self.mejorScore = 0
            }
            
            print("👤 Edad: \(self.edadUsuario ?? 9), Mejor Score MenteExacta: \(self.mejorScore)")
    ...
    private func actualizarMejorScore() {
        ...
        docRef.setData(["score_math": self.mejorScore], merge: true) { error in
            if let error = error {
                print("❌ ERROR al guardar score: \(error.localizedDescription)")
            } else {
                print("✅ Score de MenteExacta guardado: \(self.mejorScore)")
            }
        }
    }
...
//cargar pregutnas
    func cargarPreguntasLetrinas() {
                    ...
                    // rango ampliado de edades 11
                            let edadMatch = pregunta.edad.contains(where: { edadesPermitidas.contains($0) })
                            
                            let temaMatch = pregunta.tema == "Exact Mind" ||
                                           pregunta.tema == "Exakter Verstand" ||
                                           pregunta.tema == "Mente Exacta"
                            
                            return idiomaMatch && edadMatch && temaMatch
                        }
                        
                        if filtradas.isEmpty {
                            self.errorMessage = "No hay preguntas de Mente Exacta disponibles para idioma \(self.idiomaUsuario) y edad \(edadActual)"
                            self.isLoading = false
                            return
                        }
...
struct MENTEEXACTAVIEW: View {
    @StateObject private var vm = MenteViewModel()
    @StateObject private var musica = MusicaExacta()
...
//fondos
    ...
    private func fondoDinamico() -> some View {
        ...
        let fondoCase = FondoMente.fondo(para: tema, gameOver: vm.gameOver)
        ...
        ZStack {
            ...
            switch fondoCase {
            case .MENTE_NORMAL:
                if let url = GIFS.GIFMENTEEXACTA() {
